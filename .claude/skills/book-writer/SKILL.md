...
The book is served via ProperDocs with the Material theme. When chapter content changes, the site configuration must stay in sync. Forgetting this breaks navigation or hides new chapters from readers.
...
3. Create a symlink in `docs/`:
   ...
   ```
4. If the chapter number changes existing chapters, renumber the affected `properdocs.yml` entries too
...
4. Renumber subsequent chapters if needed (in filenames, `properdocs.yml`, and section headings inside the files)
...
The emulator runs guest code at near-native speed by handing the virtual CPU to
a host hypervisor, falling back to TCG binary translation only when none is...

---

## 5.1 Why Acceleration Matters
### 5.1.1 The TCG Fallback

## 5.7 Try It
- Run `emulator -accel-check` to see which accelerator is available on your host

## Summary
- KVM, Hypervisor.framework, WHPX, and AEHD each back the same accelerator interface
```
...
static int _console_client_init(ControlClient client) {
    ...
}
```

**Match code block language to source file extension.** The emulator tree is mostly C++ (`.cpp`/`.h`) and C (`.c`), with Python build/test scripts, some Go (`tools/netsim`), and Java/Kotlin in host-side tooling. Use ` ```cpp `, ` ```c `, ` ```python `, ` ```go ` to match the file — never mislabel. Key tells: C++ uses `::`, `std::`, `class`; Go uses `:=`, `func (c *config)`, no semicolons; Python uses `def`, `:` blocks, no braces.
...
Build system → QEMU machine & CPU acceleration → virtual hardware → android-emu core (sensors, snapshots, control plane) → graphics pipeline → media → connectivity → UI & streaming → guest integration → infrastructure (testing, debugging)
